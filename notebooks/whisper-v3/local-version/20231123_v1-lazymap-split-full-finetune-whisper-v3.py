...
common_voice_train = pd.concat([common_voice_train, common_voice_eval.iloc[EVAL_MAX_N_FILES:]])[:TRAIN_MAX_N_FILES]
common_voice_eval = common_voice_eval.iloc[:EVAL_MAX_N_FILES]

# check size
print(f"train size: {len(common_voice_train)}")
print(f"eval size: {len(common_voice_eval)}")
...
_train_set = gen_dataset(common_voice_train)
_val_set = gen_dataset(common_voice_eval)
_test_set = gen_dataset(common_voice_test)
...
test_set = load_or_new_process(_test_set, config, "test")
...
outputs = []
for pred, actual, fp in tqdm(zip(pipe(KeyDataset(test_set, "audio"), generate_kwargs={"language":"<|th|>", "task":"transcribe"}, batch_size=16), KeyDataset(test_set, "sentence"), KeyDataset(test_set, "path"))):
    outputs.append((pred["text"], actual, fp))
