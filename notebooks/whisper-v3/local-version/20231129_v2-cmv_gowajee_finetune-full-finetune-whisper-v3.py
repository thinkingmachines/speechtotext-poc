...
# resource config
MODEL_CHECKPOINT_DIR = Path.cwd() / "mounted-model-checkpoints-2" / "whisper-large-v3"
DATASET_CACHE_DIR = Path.cwd() / "dataset-cache"
FINAL_MODEL_OUTPUT_DIR = MODEL_CHECKPOINT_DIR / "fine-tune-model"
FINAL_PROC_OUTPUT_DIR = MODEL_CHECKPOINT_DIR / "fine-tune-proc"
...
gow_train_data = list(map(lambda s: ((ls := s.split(" "))[0] + ".wav", "".join(ls[1:])), gow_train))
gow_dev_data = list(map(lambda s: ((ls := s.split(" "))[0] + ".wav", "".join(ls[1:])), gow_dev))
gow_test_data = list(map(lambda s: ((ls := s.split(" "))[0] + ".wav", "".join(ls[1:])), gow_test))
...
# Gowajee dataset
gow_train_df = (
    pd.DataFrame(gow_train_data, columns=["filename", "sentence"])
    .assign(full_path=lambda df: GOWAJEE_AUDIO_BASE_PATH / df.filename)
    .assign(sentence=lambda df: df.sentence.map(remove_punct)) 
)
gow_eval_df = (
    pd.DataFrame(gow_dev_data, columns=["filename", "sentence"])
    .assign(full_path=lambda df: GOWAJEE_AUDIO_BASE_PATH / df.filename)
    .assign(sentence=lambda df: df.sentence.map(remove_punct)) 
)
gow_test_df = (
    pd.DataFrame(gow_test_data, columns=["filename", "sentence"])
    .assign(full_path=lambda df: GOWAJEE_AUDIO_BASE_PATH / df.filename)
    .assign(sentence=lambda df: df.sentence.map(remove_punct)) 
)

common_voice_train_eval_df = pd.concat([common_voice_train, common_voice_eval])
gow_train_eval_df = pd.concat([gow_train_df, gow_eval_df])

# check size
print(f"common_voice_train_eval size: {len(common_voice_train_eval_df)}")
print(f"gow_train_eval size: {len(gow_train_eval_df)}")


# In[ ]:


common_voice_train_df, common_voice_eval_df = train_test_split(common_voice_train_eval_df, test_size=TEST_RATIO, random_state=SEED)
gow_train_df, gow_eval_df = train_test_split(gow_train_eval_df, test_size=TEST_RATIO, random_state=int(SEED * 1.5))
test_df = pd.concat([common_voice_test, gow_test_df])
...
common_voice_train_df = filter_low_size_data(common_voice_train_df, 10)
common_voice_eval_df = filter_low_size_data(common_voice_eval_df, 10)

gow_train_df = filter_low_size_data(gow_train_df, 10)
gow_eval_df = filter_low_size_data(gow_eval_df, 10)

# check size
print(f"common voice train size: {len(common_voice_train_df)}")
print(f"common voice eval size: {len(common_voice_eval_df)}")
print(f"gow train_size: {len(gow_train_df)}")
print(f"gow eval size: {len(gow_eval_df)}")
...
def is_audio_in_length_range(data):
    return MIN_INPUT_LENGTH < data["input_lengths"] < MAX_INPUT_LENGTH
...
# _val_set = gen_dataset(opp_data_test_df)
_train_set = gen_dataset(pd.concat([common_voice_train_df, gow_train_df]).sample(frac=1, random_state=int(SEED * 2.2)))
_val_set = gen_dataset(pd.concat([common_voice_eval_df, gow_eval_df]).sample(frac=1, random_state=int(SEED * 1.992)))
...
preds = pipe([str(p) for p in test_df.full_path.tolist()], generate_kwargs={"language":"<|th|>", "task":"transcribe"}, batch_size=BATCH_SIZE)
...
for pred, t in zip(preds, test_df.itertuples()):
    outputs.append((pred["text"], t.sentence, t.full_path, t.symbol))


# In[40]:


df = pd.DataFrame(outputs, columns=["pred", "actual", "fp", "symbol"])
print(df.head())
