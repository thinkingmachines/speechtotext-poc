...
opp_data_train_test_df = pd.concat([
    amm_opp_data_df[["symbol", "full_path", "sentence"]], 
    ong_test_df.assign(symbol="Interphama")[["symbol", "full_path", "sentence"]]
])

opp_data_train_df, opp_data_test_df = train_test_split(opp_data_train_test_df, stratify=opp_data_train_test_df.symbol, test_size=TEST_RATIO)


# In[35]:


opp_data_train_df.symbol.value_counts()


# In[36]:


opp_data_test_df.symbol.value_counts()
...
opp_data_train_df = filter_low_size_data(opp_data_train_df, 10)
opp_data_test_df = filter_low_size_data(opp_data_test_df, 10)
gow_df = filter_low_size_data(gow_df, 10)

# check size
print(f"common_voice_train size: {len(common_voice_train)}")
print(f"gow_df size: {len(gow_df)}")
print(f"opp_data_train_df size: {len(opp_data_train_df)}")
print(f"opp_data_test_df size: {len(opp_data_test_df)}")
...
_oppday_set = gen_dataset(opp_data_train_df)
